...
const App = () => {
    ...
    const [pageRanks, setPageRanks] = useState(null);
    ...
    const getPageRank = (event) => {
        ...
        const keysSorted = Object.keys(res).sort(function(a,b){return res[a]-res[b]})
        setPageRanks(keysSorted);
    ...
    return (
        ...
        {pageRanks && (
            ...
            <p> Ranked Nodes </p>
            <ul>
                {pageRanks.map((pageRank) => <li key={pageRank}>{pageRank}</li>)}
            </ul>
            </div>
        )}
        </div>
